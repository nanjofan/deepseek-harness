...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all; [`dsh-desktop-app`](../packages/bundle/desktop-app/README.md) adds the Electron surface over web-app — the same browser roster with the HTTP server and WebSocket downlinks replaced by an in-process IPC carrier ([architecture note](../.agents/notes/implemented/architecture/2026-08-15-desktop-electron-shell.md)).
