...
flowchart LR
  ...
  svc_desktopRuntime["ctx.desktopRuntime<br/>Desktop in-process runtime"]
...
| `ctx.desktopRuntime` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | `connection` | - | Electron 主进程面向组合客户端图的接口：进程内 API fetch、bundle 源码读取，以及 mux/host 事件流；preload 桥驱动 dsh-client-connection 中的 DesktopApiClient。 |
