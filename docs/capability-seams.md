...
| `ctx.desktopRuntime` | `core` | [`desktop-app`](../packages/bundle/desktop-app) | - | `connection` | - | Electron main-process face over the composed client graph: in-process API fetch, bundle source reads, and the mux/host event streams; the preload bridge drives DesktopApiClient in dsh-client-connection. |
